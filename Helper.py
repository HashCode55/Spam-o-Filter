...
def word_count(training_set):
	...
	for ts, is_spam in training_set:
...
def word_probability(training_set, k = 0.5, total_spams, total_non_spams):
	count_words = word_count(training_set)
	return (w,
		(spam + k) / (2*k + total_spams), 
		(non_spams + k) / (2 * k / total_non_spams)

		for w, (spam, non_spams) in count_words.iteritems )
...
def spam_probability(word_probability, message):
	...
	for word, spam_prob, notspam_prb in word_probability:
		...
		if word in word_in_message:
			...
			log_prob_spam += math.log(1 - spam_prob) 	
			log_prob_notspam += math.log(1 - notspam_prb)
